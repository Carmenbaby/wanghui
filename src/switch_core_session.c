...
//-----------2018-05-18 start
SWITCH_DECLARE(uint32_t) switch_core_session_poc_hupall_matching_var_ans(const char *var_name, const char *var_val, switch_call_cause_t cause, _In_ const char *val_a,_In_ const char *val_b,_In_ const char *val_c,_In_ const char *val_video,switch_hup_type_t type)
{
	...
	for(np = head; np; np = np->next) {
		if ((session = switch_core_session_locate(np->str))) {
			const char *this_val = NULL;
			const char *c_number = NULL;
			const char *poc_mode = NULL;
			int ret;
			if (switch_channel_up_nosig(session->channel) &&
				...
				c_number = switch_channel_get_variable(session->channel, "poc_originate_number");
				if(c_number == NULL)
				{
					c_number = switch_channel_get_variable(session->channel, "caller_id_number");
				}
				poc_mode = switch_channel_get_variable(session->channel, "poc_mode");//conference_auto_outcall_caller_id_number
				ret = strcmp(c_number,this_val); //0 is caller // !0 is called
	
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "i_a = %d \n",i_a);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "i_b = %d \n",i_b);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "i_c = %d \n",i_c);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "i_video = %d \n",i_video);
				
				if(c_number != NULL)
				{
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "poc_originate_number = %s \n",c_number);
				}

				if(poc_mode != NULL)
				{
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "poc_mode = %s \n",poc_mode);
				}
				
			
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "iret = %d \n",ret);

				if(poc_mode != NULL)
				{
					if( 0 == strcmp(poc_mode,"a") )
					{
						// 3 all barr  1 outing barr 2 coming barr
						if( (  (!ret) && ((i_a == 3) || (i_a == 1)) ) || ( (ret) && ((i_a == 3) || (i_a == 2)) ) )
						{
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "diaplan a hangup \n");
					...
					if( 0 == strcmp(poc_mode,"b") )
					{
						// 3 all barr  1 outing barr 2 coming barr
						if( (  (!ret) && ((i_b == 3) || (i_b == 1)) ) || ( (ret) && ((i_b == 3) || (i_b == 2)) ) )
						{
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "diaplan b hangup \n");
							switch_channel_hangup(session->channel, cause);
							r++;
						}
					...
					if( 0 == strcmp(poc_mode,"c") )
					{
						// 3 all barr  1 outing barr 2 coming barr
						if( (  (!ret) && ((i_c == 3) || (i_c == 1)) ) || ( (ret) && ((i_c == 3) || (i_c == 2)) ) )
						{
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "diaplan c hangup \n");
							switch_channel_hangup(session->channel, cause);
							r++;
						}
				...
				{
						// 3 all barr  1 outing barr 2 coming barr
						if( (  (!ret) && ((i_video == 3) || (i_video == 1)) ) || ( (ret) && ((i_video == 3) || (i_video == 2)) ) )
						{
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "diaplan c hangup \n");
							switch_channel_hangup(session->channel, cause);
							r++;
