...
static void video_bridge_thread(switch_core_session_t *session, void *obj)
{
	...
	int no_recv_video = switch_true(switch_channel_get_variable(b_channel, "no_receive_video")) ||
		switch_true(switch_channel_get_variable(channel, "peer_no_receive_video"));
	...
	while (switch_channel_up_nosig(channel) && switch_channel_up_nosig(b_channel) && vh->up == 1) {
		...
		if (switch_channel_media_up(b_channel) && !no_recv_video) {
