...
//-----------2018-05-18 start
SWITCH_STANDARD_API(poc_hupall_api_function)
{
	...
	if (zstr(var)) {
		...
		switch_core_session_poc_hupall_matching_var(var, val, cause,val_a,val_b,val_c,val_video);
	}

	if (zstr(var)) {
		stream->write_function(stream, "+fail var is null \n");
		//stream->write_function(stream, "+OK hangup all channels with cause %s\n", switch_channel_cause2str(cause));
	} else {
		stream->write_function(stream, "+OK hangup all channels matching [%s]=[%s] with cause: %s\n", var, val, switch_channel_cause2str(cause));
	}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_commands_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	//2018-05-18 start
	SWITCH_ADD_API(commands_api_interface, "poc_hupall", "poc_hupall", poc_hupall_api_function, "<cause> [<var> <value> <value_a> <value_b> <value_c> <value_video>]");
	//2018-05-18 end
