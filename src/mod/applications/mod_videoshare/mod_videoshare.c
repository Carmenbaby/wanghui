...
SWITCH_MODULE_LOAD_FUNCTION(mod_videoshare_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_videoshare_shutdown);
SWITCH_MODULE_DEFINITION(mod_videoshare, mod_videoshare_load, mod_videoshare_shutdown, NULL);
SWITCH_STANDARD_APP(videoshare_function);

const char *mod_conference_app_name = "videoshare";
char *mod_conference_cf_name = "videoshare.conf";
...
//////////////////////////////////////   2017-6-19   Start   //////////////////////////////////////
SWITCH_STANDARD_API(video_switch_api_main){
	return video_switch_api_main_real(cmd, session, stream);
}

SWITCH_STANDARD_API(add_members_api_main){
	return add_members_api_main_real(cmd, session, stream);
}

// 剔出某个成员 例如：kickaddmember 会议名称 18930010001
SWITCH_STANDARD_API(videoshare_kick_api_main){
	return videoshare_kick_api_main_real(cmd, session, stream);
}

// // 来电拒接
SWITCH_STANDARD_API(videoshare_reject_api_main){
	return videoshare_reject_api_main_real(cmd, session, stream);
}
///////////////////////////////////////////      2017-6-19  end   /////////////////////////////////
...
void *SWITCH_THREAD_FUNC conference_thread_run(switch_thread_t *thread, void *obj)
{
	...
	while (conference_globals.running && !conference_utils_test_flag(conference, CFLAG_DESTRUCT)) {
		...
		if(conference->count == 1) {
			if(conference->videoshare_last_member_time == 0) {
				conference->videoshare_last_member_time = switch_epoch_time_now(NULL);
			}
			else if(switch_epoch_time_now(NULL) - conference->videoshare_last_member_time > conference->last_member_exiting_time) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Conference has only 1 member for over 5 seconds, terminating\n");
			...
			conference->videoshare_last_member_time = 0;
		}
		/////////////////////////////////////////////////////////////////////////////////////////
		////////////////////////     2017-7-11   end  /////////////////////////////////
								   ...
								   switch_call_cause_t *cancel_cause, switch_event_t *var_event)
{
	...
	*cause = SWITCH_CAUSE_NORMAL_CLEARING;
	...
	////////////////////////   2017-7-6   start   //////////////////////
	// 会议外呼时，应跳过已经加入会议的成员。
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "conference_outcall function coming in\n");
	...
	if (status != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Cannot create outgoing channel  19 fail, cause: %s\n",
		...
		//////////////  2017-7-19   start ////////////////
		// 
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "2017-7-19 Cannot create outgoing channel, cause: %s conference->count = %d conference->name = %s\n",
						  switch_channel_cause2str(*cause), conference->count, conference->name);

		if(conference->count == 1)
		{
			switch_snprintf(s_tmpbuf, sizeof(s_tmpbuf), "%s", switch_channel_cause2str(*cause));
			ss_tmp = strstr( s_tmpbuf, "REJECTED");
			if(ss_tmp == NULL)
			{
				// 说明是被呼的人无法进入会议，则这次会话将结束
				conference_utils_set_flag(conference, CFLAG_DESTRUCT);
			}
...
conference_obj_t *conference_new(char *name, conference_xml_cfg_t cfg, switch_core_session_t *session, switch_memory_pool_t *pool)
{
	...
	///////////////////   2017-7-11  START  ////////////////
	conference->last_member_exiting_time = 15;
...
SWITCH_MODULE_LOAD_FUNCTION(mod_videoshare_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	SWITCH_ADD_API(api_interface, "videoshare", "video share module commands", conference_api_main, p);
	//////////////////////////////////////   2017-6-19   Start   //////////////////////////////////////
	SWITCH_ADD_API(api_interface, "videoswitch", "video switch module commands", video_switch_api_main, p);
	SWITCH_ADD_API(api_interface, "addmembers", "add members module commands", add_members_api_main, p);
	SWITCH_ADD_API(api_interface, "videosharereject", "video share reject module commands", videoshare_reject_api_main, p);  // 来电拒接
	// videosharekick 踢出这个成员的操作
	SWITCH_ADD_API(api_interface, "videosharekick", "video switch module commands kick member", videoshare_kick_api_main, p); 
	
	
	///////////////////////////////////////////      2017-6-19  end   /////////////////////////////////
	SWITCH_ADD_APP(app_interface, mod_conference_app_name, mod_conference_app_name, NULL, videoshare_function, NULL, SAF_NONE);
	SWITCH_ADD_APP(app_interface, "videoshare_set_auto_outcall", "videoshare_set_auto_outcall", NULL, conference_auto_function, NULL, SAF_NONE);
