 ...
 *
 */
#include <mod_videoshare.h>

static const char VIDEOSHARE_SERVER_EVENT_SMS_FROM[] = "SERVER_MESSAGE@VIDEOSHARE";
...
//////////////////////////////////////   2017-6-19   Start   //////////////////////////////////////
/* _In_opt_z_ const char *cmd, _In_opt_ switch_core_session_t *session, _In_ switch_stream_handle_t *stream */
switch_status_t video_switch_api_main_real(const char *cmd, switch_core_session_t *session, switch_stream_handle_t *stream)
{
	char *lbuf = NULL;
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	char *http = NULL, *type = NULL;
	...
	if (argc == 2 && argv[0] && argv[1]) {
		conference_obj_t *conference = NULL;
		stream->write_function(stream, "video_switch_api_main_real.   %d     argv[0] = %s  argv[0] = %s\n", argc, argv[0], argv[1]);
		...
		//		char to_uri[256];
				...
				////////////////////////////  2017-7-5  START //////////////////////
				memset(&conference->m_videosharenumber , 0, sizeof(conference->m_videosharenumber)); // 视频被分享的终端号码
				////////////////////   2017-7-5   END    ///////////////////////////
				...
				for (member = conference->members; member; member = member->next) {
					if(member->session && (channel = switch_core_session_get_channel(member->session))) 
					{
						...
						// switch_snprintf(to_uri, sizeof(to_uri), "%s", profile->caller_id_number);
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "get number cmp profile->caller_id_number = %s   argv[1] = %s\n", profile->caller_id_number, argv[1]);


						if (strcasecmp(argv[1], profile->caller_id_number) == 0)
						{
							
					//		switch_snprintf(conference->m_videosharenumber, sizeof(conference->m_videosharenumber), "%s", profile->caller_id_number);
							
							// 执行切换画面的操作 
							conference_api_sub_vid_floor(member, NULL, "force");
					//		conference_api_sub_vid_floor(member, NULL, "vid-floor");

					//		strcpy( conference->m_videosharenumber, argv[1]);
							strcpy( conference->m_videosharenumber, profile->caller_id_number);
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "conference->m_videosharenumber = %s \n", conference->m_videosharenumber);
							break;
						}
					}
				}
...
// 剔出某个成员 例如：kickaddmember 会议名称 18930010001
switch_status_t videoshare_kick_api_main_real(const char *cmd, switch_core_session_t *session, switch_stream_handle_t *stream)
{
		char *lbuf = NULL;
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	char *http = NULL, *type = NULL;
	...
	if (argc == 2 && argv[0] && argv[1]) {
		conference_obj_t *conference = NULL;
		stream->write_function(stream, "videoshare_kick_api_main_real.   %d     argv[0] = %s  argv[0] = %s\n", argc, argv[0], argv[1]);
		...
		//		memset(&conference->m_videosharenumber , 0, sizeof(conference->m_videosharenumber)); // 视频被分享的终端号码
				...
				for (member = conference->members; member; member = member->next) {
					...
					//		switch_snprintf(conference->m_videosharenumber, sizeof(conference->m_videosharenumber), "%s", profile->caller_id_number);  // 2017-10-30 这句话不能运行，在这如果运行的话会导致整个会议退出
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "conference->m_videosharenumber = %s \n", conference->m_videosharenumber);
							// 踢出这个成员的操作
...
switch_status_t add_members_api_main_real(const char *cmd, switch_core_session_t *session, switch_stream_handle_t *stream)
{
	char *lbuf = NULL;
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	char *http = NULL, *type = NULL;
	...
	if (argc == 2 && argv[0] && argv[1]) {
		conference_obj_t *conference = NULL;
		stream->write_function(stream, "add_members_api_main_real.   %d     argv[0] = %s  argv[1] = %s\n", argc, argv[0], argv[1]);
...
*/
				...
				//  1  解析 参数 argv[1]  以；隔开 然后发送消息过去  让终端主动发起进入会议的请求
				...
				{     
			//	   printf("%s\n",p);    
					memset(sc_buffer, 0, 20);
				   strcpy(sc_buffer,p);
			//	   filelog_write( p ); 
			//	   filelog_write( result[si-1] );							   
				   p=strtok(NULL,d);   
					//  2  发送 sms 消息给终端 
					stream->write_function(stream, "add_members_api_main_real.  sc_buffer = %s \n",  sc_buffer);
				   conference_send_message_joinconference( argv[0],sc_buffer, member->session,stream);
				} 
				//  2  发送 sms 消息给终端
...
//  来电拒接
switch_status_t videoshare_reject_api_main_real(const char *cmd, switch_core_session_t *session, switch_stream_handle_t *stream)
{
	char *lbuf = NULL;
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	char *http = NULL, *type = NULL;
	...
	if (argc == 1 && argv[0] ) {
		conference_obj_t *conference = NULL;
		stream->write_function(stream, "videoshare_reject_api_main_real.   %d     argv[0] = %s  argv[1] = %s\n", argc, argv[0] );
...
void conference_send_message_joinconference(char *conferencename, char *joinnumber, switch_core_session_t *psession, switch_stream_handle_t *stream)
{
	switch_channel_t *channel;
	...
	memset(&to_message , 0 ,sizeof(to_message));

	stream->write_function(stream, "conference_send_message_joinconference in  111 \n");
	if(psession)
	{
		 channel = switch_core_session_get_channel(psession);
		stream->write_function(stream, "conference_send_message_joinconference in  222 \n");
		if(channel)
		{
			stream->write_function(stream, "conference_send_message_joinconference in  333 \n");
			switch_snprintf(to_uri, sizeof(to_uri), "%s@%s", joinnumber, switch_channel_get_variable(channel, "domain_name"));
			switch_snprintf(to_message, sizeof(to_message), "videoshare:%s", conferencename);
			...
			switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from", VIDEOSHARE_SERVER_EVENT_SMS_FROM);
			switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from_full", VIDEOSHARE_SERVER_EVENT_SMS_FROM);
			...
			switch_event_add_body(evt, to_message);
			// for debug
		//	switch_log_printf( psession, SWITCH_LOG_INFO, " conference_send_message_joinconference: %s  to %s\n", to_message,to_uri);
			switch_event_fire(&evt);
			stream->write_function(stream, "conference_send_message_joinconference in  444 \n");
			stream->write_function(stream, "conference_send_message_joinconference in  to_uri = %s \n", to_uri);
		}
		else
		{
			stream->write_function(stream, "  channel not found\n");
		}
	}
	else
	{
		stream->write_function(stream, "  session not found\n");
	}
...
switch_status_t conference_api_sub_list(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
	...
	if (!count) {
		stream->write_function(stream, "No active videoshare.\n");
	}
