...
////////////2017-6-29 start  //////////
// 
// static const char VIDEOSHARE_SERVER_EVENT_SMS_FROM[] = "SERVER_MESSAGE@RICHTALK";
...
struct _mapping control_mappings[] = {
	...
	{"deaf on", videoshare_loop_deaf_toggle},
	{"deaf off", videoshare_loop_undeaf_toggle},
...
///////////////////////////////////////////   2017-10-17  start    ////////////////////////////////////////
//    deaf on
void videoshare_loop_deaf_toggle(conference_member_t *member, caller_control_action_t *action)
{
	if (member == NULL)
		return;
		if (conference_utils_member_test_flag(member, MFLAG_CAN_HEAR)) {
...
//   undeaf deaf off
void videoshare_loop_undeaf_toggle(conference_member_t *member, caller_control_action_t *action)
{
	if (member == NULL)
		return;
...
void conference_send_all_message(conference_member_t *member, conference_obj_t *conference, const char *message)
{
	...
	for(imember = conference->members; imember; imember = imember->next) {
		if(imember->session && (channel = switch_core_session_get_channel(imember->session))) {
			...
			// for debug
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "  videoshare_send_all_message content: %s  to %s\n", message,to_uri);
