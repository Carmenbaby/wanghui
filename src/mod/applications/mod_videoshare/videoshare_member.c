...
// static const char VIDEOSHARE_SERVER_EVENT_SMS_FROM[] = "SERVER_MESSAGE@VIDEOSHARE";
 static const char VIDEOSHARE_SERVER_EVENT_MEMBERJOIN[] = "VIDEOSHARE_MEMBERJOIN";
 static const char VIDEOSHARE_SERVER_EVENT_MEMBERQUIT[] = "VIDEOSHARE_MEMBERQUIT";
// static const char VIDEOSHARE_SERVER_EVENT_MEMBERJOIN[] = "RICHTALK_PTT_MEMBERJOIN";
// static const char VIDEOSHARE_SERVER_EVENT_MEMBERQUIT[] = "RICHTALK_PTT_MEMBERQUIT";

#define DISPATCHER_NUMBER_PREFIX "400"  //调度员号码前缀
////////////2017-4-13  end  ///////////
...
switch_status_t conference_member_add(conference_obj_t *conference, conference_member_t *member)
{
	...
	if (conference_utils_member_test_flag(member, MFLAG_JOIN_VID_FLOOR)) {
		...
		if (test_eflag(conference, EFLAG_FLOOR_CHANGE)) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "videoshare %s OK video floor %d %s\n",
							  conference->name, member->id, switch_channel_get_name(member->channel));
		}
	}

	////////////////////////////           2017-6-29  start          //////////////////////////////
	//鍚戞墍鏈夋垚鍛樺彂閫佹柊鎴愬憳鍔犲叆浜嬩欢.
	if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
		channel = switch_core_session_get_channel(member->session);
		prof = switch_channel_get_caller_profile(channel);
		switch_snprintf(msg, sizeof(msg), "%s:%s:%s", VIDEOSHARE_SERVER_EVENT_MEMBERJOIN, conference->name, prof->caller_id_number);
...
//	switch_bool_t flag_lost_ptt_floor = SWITCH_FALSE;
	...
	if(member->session) {
		if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
			...
			switch_snprintf(msg, sizeof(msg), "%s:%s:%s", VIDEOSHARE_SERVER_EVENT_MEMBERQUIT, conference->name, caller_profile->caller_id_number);
	...
	//////////////////////////////////////   2017-7-5   start   ////////////////////////////////////
	// 1、判断当前退出的成员是不是调度员
    // 2、判断当前退出的成员是不是被分享视频的终端
    // 3、判断当前在会议里面的成员是否大于等于1人
    // 4、以上1、2的条件满足之一且3的条件满足则执行解散会议的操作
	//  sc_number[100] = {0};  // 存放退出成员终端号码
	if(!strncmp(sc_number, DISPATCHER_NUMBER_PREFIX, strlen(DISPATCHER_NUMBER_PREFIX)))
	{  // 说明是调度员
		if(conference->count >= 1) 
		{
			conference_utils_set_flag(conference, CFLAG_DESTRUCT);
...
// m_videosharenumber
	else if(!strcmp(sc_number, conference->m_videosharenumber))
	{  // 说明是视频被分享的终端号码
		if(conference->count >= 1) 
		{
			conference_utils_set_flag(conference, CFLAG_DESTRUCT);  // 本来是直接退出的，现在改成2秒后退出
