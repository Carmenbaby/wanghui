...
////////////2017-4-13 start  //////////
// RichTalk Ptt Server Event
static const char RICHTALK_SERVER_EVENT_SMS_FROM[] = "SERVER_MESSAGE@RICHTALK";
static const char RICHTALK_SERVER_EVENT_PTT_BEGINTALK[] = "RICHTALK_PTT_BEGINTALK";
static const char RICHTALK_SERVER_EVENT_PTT_ENDTALK[] = "RICKTALK_PTT_ENDTALK";
static const char RICHTALK_SERVER_EVENT_PTT_MEMBERJOIN[] = "RICHTALK_PTT_MEMBERJOIN";
static const char RICHTALK_SERVER_EVENT_PTT_MEMBERQUIT[] = "RICHTALK_PTT_MEMBERQUIT";
static const char RICHTALK_SERVER_EVENT_PTT_INFODETAIL[] = "RICHTALK_PTT_INFO_DETAIL";
static const char RICHTALK_SERVER_EVENT_PTT_INFOCOUNT[] = "RICHTALK_PTT_INFO_COUNT";
static const char RICHTALK_SERVER_EVENT_PTT_INFOPTT[] = "RICHTALK_PTT_INFO_PTT";
static const char RICHTALK_SERVER_EVENT_PTT_MEMBERONLY[] = "RICHTALK_PTT_MEMBERONLY";
////////////2017-4-13  end  ///////////
...
////////////////////////  17-4-15  start       //////////////////////////////////////
SWITCH_STANDARD_API(ptt_api_main){
	return conference_api_main_real(cmd, session, stream);
}
/////////////////////////    17-4-15  end     ///////////////////////////////////////////
...
//////////////////////// 2017-10-27  start //////////////////////////////////
void conference_memberlist(conference_obj_t *conference, switch_stream_handle_t *stream, char *delim)
{
	conference_member_t *member = NULL;
...
void *SWITCH_THREAD_FUNC conference_thread_run(switch_thread_t *thread, void *obj)
{
	...
	while (conference_globals.running && !conference_utils_test_flag(conference, CFLAG_DESTRUCT)) {
		...
		if (floor_holder != conference->floor_holder) {
			...
			conference_member_set_floor_holder(conference, floor_holder);  有些地方在调用这个的时候会加锁，所以我这也加锁测试下
			   主要原因：
			第一种情况  
			#0  __GI___pthread_mutex_lock (mutex=0x0) at ../nptl/pthread_mutex_lock.c:66
			#1  0x00007fd33f171da9 in apr_thread_mutex_lock (mutex=<optimized out>) at locks/unix/thread_mutex.c:92
			#2  0x00007fd33ee69cd5 in switch_mutex_lock (lock=<optimized out>) at src/switch_apr.c:287
			#3  0x00007fd33eeefcd9 in switch_live_array_lock (la=<optimized out>) at src/switch_event.c:3463
			#4  0x00007fd32fb06cd7 in conference_member_update_status_field (member=0x7fd331735410) at conference_member.c:170
			#5  0x00007fd32fb08293 in conference_member_set_floor_holder (conference=0x7fd2bd0ced68, member=0x23) at conference_member.c:1100
			#6  0x00007fd32fae3c20 in conference_thread_run (thread=0xda5af8ffda5af907, obj=0x7fd2bd0ced68) at mod_conference.c:379
			#7  0x00007fd33f177f30 in dummy_worker (opaque=0x7fd2bd0cf860) at threadproc/unix/thread.c:151
			#8  0x00007fd33e3b5064 in start_thread (arg=0x7fd33862b700) at pthread_create.c:309
			#9  0x00007fd33da8d62d in clone () at ../sysdeps/unix/sysv/linux/x86_64/clone.S:111

			第二种情况
			(gdb) bt
			#0  0x00007f51c8b51c7a in conference_member_update_status_field (member=0x7f51ca093410) at conference_member.c:166
			#1  0x00007f51c8b53293 in conference_member_set_floor_holder (conference=0x7f5108644608, member=0x0) at conference_member.c:1100
			#2  0x00007f51c8b2ec20 in conference_thread_run (thread=0x7f51ca537c80, obj=0x7f5108644608) at mod_conference.c:379
			#3  0x00007f51e014df30 in dummy_worker (opaque=0x7f5108645100) at threadproc/unix/thread.c:151
			#4  0x00007f51df38b064 in start_thread (arg=0x7f51ca544700) at pthread_create.c:309
			#5  0x00007f51dea6362d in clone () at ../sysdeps/unix/sysv/linux/x86_64/clone.S:111
			...
			*/    
			...
			/////////////////////////////         2017-4-19    start     ////////////////////
			// for debug
		//	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "conference_thread_run: floor_holder\n");
		//	conference->ptt_floor_holder = conference->floor_holder;
		//	conference->ptt_floor_holder->ptt_gain_time = switch_epoch_time_now(NULL);
			/////////////////////////////////        2017-4-19    end    ////////////////////
		...
		//若不说话超过若干秒,则自动结束说话.
		for (imember = conference->members; imember; imember = imember->next) {
			if(imember == conference->ptt_floor_holder) {
				int is_talking_ptt = 0;
				switch_time_t current_time = switch_epoch_time_now(NULL);
				if (current_time - imember->ptt_gain_time < conference->lost_ptt_on_talking) {
				if (current_time - imember->ptt_gain_time <= conference->lost_ptt_on_silence) {
						is_talking_ptt = 1;
					} else if(imember->last_talking != 0 && current_time - imember->last_talking <= conference->lost_ptt_on_silence) {
						is_talking_ptt = 1;
					}
				}
				if(!is_talking_ptt) {
					//switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(imember->session), SWITCH_LOG_INFO, "Lost ptt floor due to time out\n");
					conference_member_t *imem;
					char msg[128], to_uri[256];
					switch_channel_t *channel, *imem_chan;
					switch_caller_profile_t *profile, *imem_prof;
					switch_event_t *evt = NULL;

					conference_api_sub_mute(imember, NULL, NULL);
					conference->ptt_floor_holder = NULL;
					imem_chan = switch_core_session_get_channel(imember->session);
					if (conference_utils_member_test_flag(imember, MFLAG_NOCHANNEL))
					{
						continue;
					}
					imem_prof = switch_channel_get_caller_profile(imem_chan);
					switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_ENDTALK, imem_prof->caller_id_number);

					//conference_send_all_message(imember, conference, msg);
					for(imem = conference->members; imem; imem = imem->next) {
						if(imem->session && (channel = switch_core_session_get_channel(imem->session))) {
							if (conference_utils_member_test_flag(imem, MFLAG_NOCHANNEL))
							{
								continue;
							}
							profile = switch_channel_get_caller_profile(channel);
							switch_snprintf(to_uri, sizeof(to_uri), "%s@%s", profile->caller_id_number, switch_channel_get_variable(channel, "domain_name"));
							switch_event_create_subclass(&evt, SWITCH_EVENT_CUSTOM, "SMS::SEND_MESSAGE");
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "proto", "GLOBAL_SMS");
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from_proto", CONF_CHAT_PROTO);
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "dest_proto", "sip");
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from", RICHTALK_SERVER_EVENT_SMS_FROM);
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from_full", RICHTALK_SERVER_EVENT_SMS_FROM);
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "to", to_uri);
							switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "type", "text/plain");
							switch_event_add_body(evt, msg);
		...
		//当会议只剩最后一个成员时,在若干秒内结束会议.
	//	if((conference->count == 1 && conference->id_senior_disp == 0)|| ( conference->count == 2 && conference->id_senior_disp > 0 )) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference->count =  %d seconds, conference->senior_disp_Count = %d\n", conference->count, conference->senior_disp_Count);

		if(conference->count == 1 || ( conference->count -  conference->senior_disp_Count <= 1 )) {
			if(conference->ptt_last_member_time == 0) {
				conference->ptt_last_member_time = switch_epoch_time_now(NULL);
			}
			else if(switch_epoch_time_now(NULL) - conference->ptt_last_member_time > conference->last_member_exiting_time) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has only 1 member for over %d seconds, terminating\n", conference->last_member_exiting_time);
				// conference_utils_set_flag
				if(!conference_utils_test_flag(conference, CFLAG_DESTRUCT))
				{
				//	conference_member_t *member;
					
					//  poc_call_number 这个变量如果在单呼的情况下是不会有值的 这个在拨号规则里面没有加 只要预定义和临时组才会加
					if((strlen(conference->poc_call_number) > 0) && (!conference->m_b_count)) // 用来标记这次会议有没有超过 大于等于2个人      == 1 有  == 0  没有
					{
						char msg3[256] = {0};
						switch_snprintf(msg3, sizeof(msg3), "%s:%s", RICHTALK_SERVER_EVENT_PTT_MEMBERONLY, conference->poc_call_number);
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has only 1 member for over %d seconds, terminating\n", conference->last_member_exiting_time);

						conference_send_all_message(NULL, conference, msg3);
					...
					//   2017-11-20 暂时问题：这边发现如果只剩最后一个成员的时候，好像发送quit的消息发不出去  
					//    看下要不要先发送退出消息再发送销毁事件
					/*////////////////////////////////////////////////////////////////////////////////////
					/////////////////////////////////////////////////////////////////////////////////////

					for (member = conference->members; member; member = member->next) {
						conference_api_sub_kick(member, NULL, NULL);
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has only 1 member for over 111  %d seconds, terminating\n", conference->last_member_exiting_time);
					}
					member = NULL;
					/////////////////////////////////////////////////////////////////////////////////////
					////////////////////////////////////////////////////////////////////////////////////*/
			...
			conference->ptt_last_member_time = 0;
		}
		/////////////////////////////////////////////////////////////////////////////////////////
		////////////////////////     2017-4-19   end  /////////////////////////////////
		...
		// for debug
		// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has been idle for over %d seconds, \n", conference->terminate_on_silence);
		...
		if (conference->terminate_on_silence && conference->count > 1) {
			...
			// for debug 
			// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has been idle for over %d seconds, terminating   is_talking = %d\n", conference->terminate_on_silence,is_talking);
			if (is_talking == 0) {
				...
				//	conference_member_t *member;
					...
					for (member = conference->members; member; member = member->next) {
						conference_api_sub_kick(member, NULL, NULL);
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Conference has been idle for over %d seconds kick , terminating\n", conference->terminate_on_silence);
					}
					member = NULL;
					/////////////////////////////////////////////////////////////////////////////////////
					////////////////////////////////////////////////////////////////////////////////////*/
								   ...
								   switch_call_cause_t *cancel_cause, switch_event_t *var_event)
{
	...
	///////////////////////////////////       END       ////////////////////////////////////
	/////////////////////////////   2017-4-18   START   ////////////////////////////////////
	char *p, *p_end, *dial_str_end;
	conference_member_t *tmember;
	switch_channel_t *testchan;
	switch_caller_profile_t *testprof;
	int is_duplicated_member = 0;
	switch_status_t ret_status = SWITCH_STATUS_FALSE;
//	switch_channel_t *channel;
//	call_list_t *call_list;
	char sc_number_tmp[512] = "";
	strcpy( sc_number_tmp, bridgeto);
	///////////////////////////////////       END       ////////////////////////////////////
...
////////////////////////////////  2017-4-18  START  //////////////////////
	...
	/////////////////////////////////////////////////////////////////////////////////////////////   2017-5-25   start    ////////////////////
//	switch_time_t current_time = switch_epoch_time_now(NULL);
//	if (current_time - conference->checkMember_time >= 2)  // 超过2秒
//	{
		// SWITCH_STATUS_SUCCESS 0 否则返回 1  SWITCH_STATUS_FALSE
/*		
		if(!caller_channel)
		{
			if (call_list = switch_channel_get_private(caller_channel, "_conference_autocall_list_")) 
			{
				int sigroup_priorty = 0;
				const char *sgroup_priorty = switch_channel_get_variable(caller_channel, "conference_auto_outcall_group_priorty"); // 获取组间优先级
				
				if (!zstr(sgroup_priorty)) 
				{
					sigroup_priorty = atoi(sgroup_priorty);  // 组间优先级
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
					"conference_outcall sgroup_priorty.   value =  %d\n", sigroup_priorty);
				}
				else
				{
					sigroup_priorty = 1;
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
					"conference_outcall sgroup_priorty.  no value\n");
				}
		
			}
		}
*/
                                                 // 参数说明  会议句柄         会议的优先级            要加入的成员号码

		switch_mutex_lock(conference->check_member_mutex);
		
		ret_status = conference_check_member_outcall( conference, conference->group_priorty, bridgeto);
		
	//	ret_status = 1;  // for debug
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			"ret_status = %d .\n", ret_status);
//		if(conference->check_status)  // status = 2;  // 说明当前正在通话的优先级是比较高的，不需要退出
		if(ret_status == 2)  // status = 2;  // 说明当前正在通话的优先级是比较高的，不需要退出
		{

			//////////////////////////////    2017-11-23  start    ///////////////////////////////////
			//2017-11-21  start
			if(conference->m_b_singecall)   // 用来标记这次会议 是不是单呼  1 - 单呼  0 - 不是单呼
			{
				switch_snprintf(msg, sizeof(msg), "%s:USER_BUSY", RICHTALK_SERVER_EVENT_SMS_FROM);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "2017-11-21 22222 error cause: %s\n", msg);
				switch_mutex_lock(conference->member_mutex);

				caller_channel = switch_core_session_get_channel(session);
				if(caller_channel) 
				{
					testprof = switch_channel_get_caller_profile(caller_channel);
					switch_snprintf(to_uri, sizeof(to_uri), "%s@%s", testprof->caller_id_number, switch_channel_get_variable(caller_channel, "domain_name"));
					switch_event_create_subclass(&evt, SWITCH_EVENT_CUSTOM, "SMS::SEND_MESSAGE");
					switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "proto", "GLOBAL_SMS");
					switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from_proto", CONF_CHAT_PROTO);
					switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "dest_proto", "sip");
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
			"Conference outcall2 dial string: %s duplicates with existing member, outcall canceled.\n", bridgeto);
			if (caller_channel) {
				switch_channel_hangup(caller_channel, *cause);  // guess this only with bridged call
			}
			status = SWITCH_STATUS_FALSE;
			...
			switch_mutex_unlock(conference->check_member_mutex);
			return status;
		//	goto done;
		}
		switch_mutex_unlock(conference->check_member_mutex);
...
//	}
	/////////////////////////////////////////////////////////////////////////////////////////////  2017-5-25 end         ////////////////////
	switch_mutex_lock(conference->member_mutex);
	for (tmember = conference->members; tmember; tmember = tmember->next) {
		if (tmember->session) {
			testchan = switch_core_session_get_channel(tmember->session);
			if (conference_utils_member_test_flag(tmember, MFLAG_NOCHANNEL))
			{
				continue;
			}
			testprof = switch_channel_get_caller_profile(testchan);
			p = strstr(bridgeto, testprof->caller_id_number);
			if (p) {
				p_end = p + strlen(testprof->caller_id_number);
				dial_str_end = bridgeto + strlen(bridgeto);
				if ((p == bridgeto || !isalnum(*(p-1))) && (p_end == dial_str_end || !isalnum(*p_end))) {
					is_duplicated_member = 1;
					break;
				}
			}
		}
	...
	if (is_duplicated_member) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
			"Conference outcall dial string: %s duplicates with existing member, outcall canceled.\n", bridgeto);
		if (caller_channel) {
			switch_channel_hangup(caller_channel, *cause);  // guess this only with bridged call
		}
		status = SWITCH_STATUS_FALSE;
...
////////////////////////////////////       END ///////////////////////////
	...
	if (status != SWITCH_STATUS_SUCCESS) {
		...
		if(conference->m_b_singecall)   // 用来标记这次会议 是不是单呼  1 - 单呼  0 - 不是单呼
		{
			switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_SMS_FROM, switch_channel_cause2str(*cause));
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "2017-11-21  error cause: %s\n", msg);
			switch_mutex_lock(conference->member_mutex);
	...
	//2018-05-18 start
	if(conference->poc_mode != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_conference.c:1827 conference->poc_mode not null\n");
		switch_channel_set_variable(peer_channel, "poc_mode", conference->poc_mode );
	}
	
	if(conference->poc_originate_number != NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_conference.c:1833 conference->poc_originate_number not null\n");
		switch_channel_set_variable(peer_channel, "poc_originate_number", conference->poc_originate_number);// abc
	}
	//2018-05-18 end
...
SWITCH_STANDARD_APP(conference_function)
{
	...
	if (isbr) {
		...
		if ((conference = conference_find(conference_name, NULL))) {
			...
			///////////////////////// 2017-4-17  start ////////////////////
			conference_member_t *testmem;
			switch_channel_t *testchan;
			switch_caller_profile_t *testprof;
			char *p, *p_end, *conf_name_end;

	//		if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL))  2017-11-13 
	//		{
				
			
				const char *mynumber = switch_channel_get_caller_profile(channel)->caller_id_number;


				// // 检查是否是重复的临时组呼实例（实例名称=呼起号码），若是，则直接结束此路呼叫。
				p = strstr(conference_name, mynumber);
				if (p ) {
					p_end = p + strlen(mynumber);
					conf_name_end = conference_name + strlen(conference_name);
					if ((p == conference_name || !isalnum(*(p-1))) && (p_end == conf_name_end || !isalnum(*p_end))) {
						switch_thread_rwlock_unlock(conference->rwlock);
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
							"Caller number %s try to re-enter a temp group ptt instance %s!\n", mynumber, conference_name);
						goto done;
					}
				}

				// 若成员已在会议中,则踢出上一个登录会话.
				for(testmem = conference->members; testmem; testmem = testmem->next) {
					if(testmem->session) {
						testchan = switch_core_session_get_channel(testmem->session);
						if ((testchan == NULL) || (conference_utils_member_test_flag(testmem, MFLAG_NOCHANNEL)))
						{
							continue;
						}
						testprof = switch_channel_get_caller_profile(testchan);
						if(!strcmp(testprof->caller_id_number, mynumber)) {
							conference_api_sub_kick(testmem, NULL, NULL);
							break;
						}
					}
				}
	...
	if (!zstr(bridgeto) && strcasecmp(bridgeto, "none")) {
		...
		//////////////////////////////////    2017-4-25   start   //////////////////////////////////////
		////////////////////////////////////////// 以下是实现单呼的
		// Bridged call seems simple here. Maybe we can add more option(chan var) for user to set...
		// For now only flags, timeout, caller id and name
		// --By zengwf
		const char *bridge_call_cid_name = switch_channel_get_variable(channel, "conference_bridged_outcall_caller_id_name");
		const char *bridge_call_cid_num = switch_channel_get_variable(channel, "conference_bridged_outcall_caller_id_number");
		const char *bridge_call_toval = switch_channel_get_variable(channel, "conference_bridged_outcall_timeout");
		const char *bridge_call_flags = switch_channel_get_variable(channel, "conference_bridged_outcall_flags");
		char *bc_cid_name, *bc_cid_num, *bc_flags;
		int bc_to = 60;
		if (bridge_call_toval) {
			bc_to = atoi(bridge_call_toval);
			if (bc_to < 10 || bc_to > 500) {
				bc_to = 60;
			}
		}
		bc_cid_name = switch_core_strdup(conference->pool, bridge_call_cid_name);
		bc_cid_num = switch_core_strdup(conference->pool, bridge_call_cid_num);
		bc_flags = switch_core_strdup(conference->pool, bridge_call_flags);

		//2018-05-18 start
		if(NULL == conference->poc_mode)
		{
			conference->poc_mode = switch_channel_get_variable(channel, "poc_mode");
		}
		if(NULL == conference->poc_originate_number)
		{
			conference->poc_originate_number = switch_channel_get_variable(channel, "poc_originate_number");
		}
	    
		if (!zstr(conference->poc_mode)) 
		{
			//strcpy(conference->poc_mode, str_poc_mode);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"singlecall poc_mode = %s\n",conference->poc_mode);
		}

		if (!zstr(conference->poc_originate_number)) 
		{
			//strcpy(conference->poc_originate_number, str_poc_originate_number);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"singlecall poc_originate_number = %s\n",conference->poc_originate_number);
		}
		//2018-05-18 end

		////////////////  2017-11-23   start  在这里设置单呼的标志位  ////////////////////////////////////
		conference->m_b_singecall = 1; // 用来标记这次会议 是不是单呼  1 - 单呼  0 - 不是单呼
		/////////////////  2017-11-23  end    ////////////////////////////////////////////////////
		
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"17-4--27  5555555555555555555555 \n");

		// 输出的日志   ====>   17-4--27  6666666666666666666666  bridgeto = user/18046347309@192.168.2.123 bc_to = 60 bc_flags = mute bc_cid_name = 18046347306 bc_cid_num = 18046347306

		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"17-4--27  6666666666666666666666  bridgeto = %s bc_to = %d bc_flags = %s bc_cid_name = %s bc_cid_num = %s\n", bridgeto, bc_to, bc_flags, bc_cid_name ,bc_cid_num);

		if (conference_outcall(conference, NULL, session, bridgeto, bc_to, bc_flags, bc_cid_name, bc_cid_num, NULL, &cause, NULL, NULL) != SWITCH_STATUS_SUCCESS) {
			goto done;
		}
	...
	*/
	...
	/////////////////////////        2017-4-25   start     ///////////////////////////
	// Set member ptt priorty from variable. currently doesn't support changing it during conference(ptt) process
	member.ptt_priorty = 1;   // default value
//	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "conference_thread_run: floor_holder\n");
	

	
	member.senior_disp_Flag = 0;    // 高级调度员标志位   1 - 是高级调度员   0 - 不是   默认值为 0  不是高级调度员
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"ptt_priorty    senior_disp_Flag = 0    11111111111111111111111111 \n");

	/////////////////////  2017-12-4 start /////////////////////////////////////////////////
	if (!conference_utils_member_test_flag(&member, MFLAG_NOCHANNEL)) {
		switch_channel_t *channel_s = NULL;
		switch_caller_profile_t *prof_s;
		char msg_s[128];


		const char *s_sen_disp  = NULL;
		channel_s = switch_core_session_get_channel(member.session);
		prof_s = switch_channel_get_caller_profile(channel_s);
		switch_snprintf(msg_s, sizeof(msg_s), "%s", prof_s->caller_id_number);
		channel_s = NULL;

		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
				"prof_s->caller_id_number  msg_s = %s\n", msg_s);
	   ...
	   //2018-05-18 start
		...
		if (!zstr(conference->poc_mode))
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"poc_mode = %s\n",conference->poc_mode);
		}

		if (!zstr(conference->poc_originate_number))
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"poc_originate_number = %s\n",conference->poc_originate_number);
		}
		//2018-05-18 end

	//	if (!zstr(ptt_priorty_str = switch_channel_get_variable(channel, "18120636937"))) 
		if (!zstr(ptt_priorty_str = switch_channel_get_variable(channel, msg_s))) 
		{
			member.ptt_priorty = atoi(ptt_priorty_str);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
				"ptt_priorty set from variable to  %s = %s\n", msg_s, ptt_priorty_str);
			conference->channel = channel;
		}
		else
		{
			if(conference->channel)
			{
		//		if (!zstr(ptt_priorty_str = switch_channel_get_variable(conference->channel, "18120636937"))) 
				if (!zstr(ptt_priorty_str = switch_channel_get_variable(conference->channel, msg_s))) 
				{
					member.ptt_priorty = atoi(ptt_priorty_str);
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						"ptt_priorty set from variable to  22222222222  %s  =  %s\n", msg_s, ptt_priorty_str);
				}
			}
		}
		////////////////////////////////  2018-4-28  start   ///////////////////////////////

		//    conference_api_sub_kick(member, NULL, NULL);  已经有一个高级调度员了   踢出这个成员
		if (!zstr(s_sen_disp = switch_channel_get_variable(channel, "senior_dispatcher")))  //  保存高级调度员号码 Senior dispatcher.
		{
			//	switch_snprintf(conference->senior_disp, sizeof(conference->senior_disp), "%s", s_sen_disp );
		//	conference->senior_disp_Count++;  //  会议人数加 1 
			member.senior_disp_Flag = 1;  //   当前调度员为高级调度员
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"2018-4-28  保存高级调度员号码     s_sen_disp = %s \n",  s_sen_disp);
		}
		////////////////////////////////  2018-4-28  end    ///////////////////////////////
	}
	///////////////////////  2017-12-4  end   //////////////////////////////////////////////

	if (!zstr(ptt_priorty_str = switch_channel_get_variable(channel, "ptt_priorty"))) 
	{
		member.ptt_priorty = atoi(ptt_priorty_str);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"ptt_priorty set from variable to %s\n", ptt_priorty_str);
	}
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"ptt_priorty 22222222222222222222222222 \n");
...
conference_obj_t *conference_new(char *name, conference_xml_cfg_t cfg, switch_core_session_t *session, switch_memory_pool_t *pool)
{
	...
	////////////////////     2017-4-17  start   ////////////
	char *lost_ptt_on_silence = NULL;
	char *lost_ptt_on_talking = NULL;
	char *last_member_exiting_time = NULL;
	char *dummy_frame_rate = NULL;
...
/////////////////////////  2017-4-17  start  /////////////
			else if (!strcasecmp(var, "lost-ptt-on-silence") && !zstr(val)) {
				lost_ptt_on_silence = val;
			} else if (!strcasecmp(var, "lost-ptt-on-talking") && !zstr(val)) {
				lost_ptt_on_talking = val;
			} else if (!strcasecmp(var, "last-member-exiting-time") && !zstr(val)) {
				last_member_exiting_time = val;
			} else if (!strcasecmp(var, "dummy-frame-send-rate") && !zstr(val)) {
				dummy_frame_rate = val;
			}
			//////////////////////////    2017-4-17  end      //////////////
		}
	...
	///////////////////   2017-4-17  START  ////////////////
	conference->checkMember_time = 0;  // 查询当前所有会议成员以及以及它们优先级
	conference->group_priorty = 1;  // 组间优先级
	conference->lost_ptt_on_silence = 20;
	conference->lost_ptt_on_talking = 60;
	conference->last_member_exiting_time = 10;
	conference->dummy_frame_send_rate = 25;
	conference->http_interface_key = NULL;
	conference->channel = NULL;

	//2018-05-18 start
	conference->poc_mode = NULL;
	conference->poc_originate_number = NULL;
	//2018-05-18 end
	
	memset(conference->poc_call_number, 0x00,sizeof(conference->poc_call_number));
	memset(conference->senior_disp, 0x00,sizeof(conference->senior_disp));  //  保存高级调度员号码 Senior dispatcher.  (senior_dispatcher)
	conference->id_senior_disp = 0;  // 保存高级调度员号码 对应在conference里面的id值  Senior dispatcher.  (senior_dispatcher)
	conference->senior_disp_Count = 0;  // 标记当前高级调度员的人数为0
	conference->m_b_count = 0;   // 用来标记这次会议有没有超过 大于等于2个人      == 1 有  == 0  没有
	conference->m_b_singecall = 0; // 用来标记这次会议 是不是单呼  1 - 单呼  0 - 不是单呼
	
	if (!zstr(lost_ptt_on_silence)) {
		conference->lost_ptt_on_silence = atoi(lost_ptt_on_silence);
	}
	if (!zstr(lost_ptt_on_talking)) {
		conference->lost_ptt_on_talking = atoi(lost_ptt_on_talking);
	}
	if (!zstr(last_member_exiting_time)) {
		conference->last_member_exiting_time = atoi(last_member_exiting_time);
	}
	if (!zstr(dummy_frame_rate)) {
		conference->dummy_frame_send_rate = atoi(dummy_frame_rate);
	}

	conference->ptt_floor_holder = NULL;
	conference->ptt_last_member_time = 0;

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "conference new 11111111111.\n");
	...
	/////////////////////    2017-5-31   start   初始化互斥锁  ////////////////////////
	switch_mutex_init(&conference->check_member_mutex, SWITCH_MUTEX_NESTED, conference->pool);
	///////////////////////  2017-5-31   end    //////////////////////////
...
SWITCH_MODULE_LOAD_FUNCTION(mod_conference_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	////////////////  17-4-13  start  conference 的别名 ////////////////////////
	SWITCH_ADD_API(api_interface, "ptt", "ptt module commands", ptt_api_main, p);
	////////////////  17-4-13  end   conference 的别名 //////////////////////////
...
////////////////////////////////////////   2017-5-25  组间优先级  start //////////////////////
// 判断当前号码的优先级，是否比较高，如果比较高 返回 SWITCH_STATUS_SUCCESS 0 否则返回 1  SWITCH_STATUS_FALSE
switch_status_t conference_check_member_outcall(conference_obj_t *conference,int GroupPriority,char *MemberNumber)
{
...
//	int ret_status = SWITCH_STATUS_GENERR;
	...
	int si_Find = 0;  //  判断是否有找到这个号码，且需要将该成员提出该组  为 1 是  为 0 不是
	int si_Find1 = 0;  //  是否直接退出两重循环， 1 - 是  0 - 不是
	conference_member_t *member = NULL;

	switch_status_t check_status = SWITCH_STATUS_SUCCESS;
//	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-24 for debug conference_list_conferences!\n");
	...
	//	int fcount = 0;
		...
		/*///////////////////////////////////////////////////////////////////////////////////
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-25 for debug Conference %s (%u member%s rate: %u%s flags: ",
								   conference->name,
								   conference->count,
	...
	//				uint32_t count = 0;
					...
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-25  %u%s%s%s%s%s%s%s%s%s",
											   member->id, d, name, d, uuid, d, profile->caller_id_name, d, profile->caller_id_number, d);

	
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-25  %s%d%s%d%s%d%s%d\n", d,
											   member->volume_in_level,
											   d,
											   member->agc_volume_in_level,
											   d, member->volume_out_level, d, member->energy_level);


					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-26  MemberNumber 被呼叫号码 = %s  profile->caller_id_number 会议中的号码 = %s\n", MemberNumber, profile->caller_id_number);

					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-5-26  member->group_priorty 成员在组的优先级 = %d member->conference->group_priorty 组的优先级= %d GroupPriority 即将加入组的优先级 = %d\n", member->group_priorty, member->conference->group_priorty, GroupPriority);
					
					p = strstr(MemberNumber, profile->caller_id_number);
			//		if (p && member->conference->group_priorty >= GroupPriority) // 先不考虑优先级

					// 若成员已在会议中,则踢出上一个登录会话.
					// conference_api_sub_kick(testmem, NULL, NULL);
					if (p )
					{
						if( member->conference->group_priorty >= GroupPriority )
						{
					//		check_status = SWITCH_STATUS_FALSE;
						...
						else
						{
						   ...
						   conference_api_sub_kick(member, NULL, NULL);  // 不知道放在这里有没有问题
							si_Find1 = 1;
							break;
						}
	...
	stream.write_function(&stream, "Comming check  kick member yes or no.\n");
	if(si_Find == 1)   //  放在这里还会有问题
	{
	   // 若成员已在会议中,则踢出上一个登录会话.
		stream.write_function(&stream, "Comming in kick member .\n");
//	   conference_api_sub_kick(member, NULL, NULL);  // 不知道放在这里有没有问题
		stream.write_function(&stream, "Comming out kick member .\n");
	//	Sleep(1);
	}
