...
api_command_t conference_api_sub_commands[] = {
	...
	{"memberlist", (void_fn_t) & conference_api_sub_memberlist, CONF_API_SUB_ARGS_SPLIT, "memberlist", "[delim <string>]|[count]"},
...
switch_status_t conference_api_main_real(const char *cmd, switch_core_session_t *session, switch_stream_handle_t *stream)
{
	...
	if (argc && argv[0]) {
		...
		if ((conference = conference_find(argv[0], NULL))) {
			...
			} else if (strcasecmp(argv[0], "memberlist") == 0) {
				conference_api_sub_memberlist(NULL, stream, argc, argv);
...
//////////////////////   2017-4-18  start ////////////////////
switch_status_t conference_api_sub_ptt_floor(conference_member_t *member, switch_stream_handle_t *stream, void *data)
{
	char msg[128];
	switch_channel_t *channel;
	switch_caller_profile_t *caller_profile;
	conference_member_t *current_floor_holder;
	...
	//for debug
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "2017-4-19_  conference_api_sub_ptt_floor\n");

	channel = switch_core_session_get_channel(member->session);

	if(channel == NULL)
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "2017-11-9_  conference_api_sub_ptt_floor channel null\n");
		return SWITCH_STATUS_SUCCESS;
	}
	//   2017-11-13
	...
	current_floor_holder = member->conference->ptt_floor_holder;
	if (current_floor_holder == member) {  // \CAͷŻ\B0\D3\EFȨ
		member->conference->ptt_floor_holder = NULL;

		switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_ENDTALK, caller_profile->caller_id_number);
		//for debug
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "2017-4-18  Sending floor lost message to members: %s\n", msg);
		conference_send_all_message(member, member->conference, msg);
	} else if (current_floor_holder == NULL) { // \B5õ\BD\BB\B0\D3\EFȨ
		member->conference->ptt_floor_holder = member;
		member->ptt_gain_time = switch_epoch_time_now(NULL);

		switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_BEGINTALK, caller_profile->caller_id_number);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "2017-4-18  Sending floor gain message to members: %s\n", msg);
		conference_send_all_message(member, member->conference, msg);
	} else if (member->ptt_priorty > current_floor_holder->ptt_priorty) { // \D3\C5\CFȼ\B6\C7л\BB\BB\B0\D3\EFȨ
		// Take up due to higher priorty
		if(current_floor_holder->ptt_priorty > 0)
		{
			/*
			  加这层if的判断主要原因是， 看下加完if后有没有效果
			  #0  switch_core_session_get_channel (session=0x100000000) at src/switch_core_session.c:1358
			  #1  0x00007f041d48c0e1 in conference_api_sub_ptt_floor (member=0x100000000, stream=0x0,   data=0x136dc68) at conference_api.c:439
			  #2  0x00007f041d48f700 in conference_loop_mute_off (member=0x100000000, action=0x0) at   conference_loop.c:515
			  #3  0x00007f041d48fb82 in conference_loop_dmachine_dispatcher (match=0x14857e8) at   conference_loop.c:111
			  #4  0x00007f042c8429ef in switch_ivr_dmachine_ping (dmachine=0x14857c0, match_p=0x0) at   src/switch_ivr_async.c:517
			  #5  0x00007f042c842cab in switch_ivr_dmachine_feed (dmachine=0x14857c0, digits=0x7f041ead4d10 "*",   match=0x0) at src/switch_ivr_async.c:616
			  #6  0x00007f041d48eb01 in conference_loop_input (thread=<optimized out>, obj=0x7f041ec00410) at   conference_loop.c:1100
			  #7  0x00007f042ca9ff30 in dummy_worker (opaque=0xffb198) at threadproc/unix/thread.c:151
			  #8  0x00007f042bcdd064 in start_thread (arg=0x7f041ead5700) at pthread_create.c:309
			  #9  0x00007f042b3b562d in clone () at ../sysdeps/unix/sysv/linux/x86_64/clone.S:111
			*/
			switch_channel_t *old_channel = switch_core_session_get_channel(current_floor_holder->session);
			if (!conference_utils_member_test_flag(current_floor_holder, MFLAG_NOCHANNEL))
			{
				switch_caller_profile_t *old_caller_profile = switch_channel_get_caller_profile(old_channel);
				conference_api_sub_mute(current_floor_holder, NULL, NULL);
				member->conference->ptt_floor_holder = member;
				member->ptt_gain_time = switch_epoch_time_now(NULL);

				switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_ENDTALK, old_caller_profile->caller_id_number);
				conference_send_all_message(member, member->conference, msg);
		...
		{
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "2017-11-16  conference_api_sub_ptt_floor current_floor_holder->ptt_priorty 优先级小于0 明显不对\n");
		}
...
///////////////   2017-10-27 add function   ///////////////////
switch_status_t conference_api_sub_memberlist(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
	int ret_status = SWITCH_STATUS_GENERR;
	int count = 0;
	char *d = ";";
	...
	//		conference_list_pretty(conference, stream);
			...
			conference_memberlist(conference, stream, d);
		}
	}

	if (!count) {
		stream->write_function(stream, "No active conference.\n");
	}
