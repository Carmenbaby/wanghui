...
static const char RICHTALK_SERVER_EVENT_PTT_MEMBERS[] = "RICHTALK_PTT_MEMBERS";
...
//	{"ptt query detail", conference_loop_mute_toggle},
//    {"ptt query count", conference_loop_mute_toggle},
//    {"ptt query ptt", conference_loop_mute_toggle},
	{"ptt query detail", conference_loop_fn_ptt_querydetail},
	{"ptt query online detail", conference_loop_fn_ptt_online_querydetail},
    {"ptt query count", conference_loop_fn_ptt_querycount},
    {"ptt query ptt", conference_loop_fn_ptt_queryfloor},
...
////////////////////////////////////////////
// 2017-4-13 �Լ���ӵ�  start ////////////////
void conference_loop_fn_ptt_querydetail(conference_member_t *member, caller_control_action_t *action)
{
	switch_stream_handle_t stream = { 0 };
	if(member == NULL) return;

	SWITCH_STANDARD_STREAM(stream);
	conference_ptt_list(member->conference, &stream);
	conference_send_message(member, member->conference, (char *)stream.data);
	switch_safe_free(stream.data);
}


void conference_loop_fn_ptt_online_querydetail(conference_member_t *member, caller_control_action_t *action)
{
	switch_stream_handle_t stream = { 0 };
	if(member == NULL) return;

	SWITCH_STANDARD_STREAM(stream);
	conference_ptt_online_list(member->conference, &stream);
	conference_send_message(member, member->conference, (char *)stream.data);
	switch_safe_free(stream.data);
}

void conference_loop_fn_ptt_querycount(conference_member_t *member, caller_control_action_t *action)
{
	char msg[128];
	if(member == NULL) return;

	///////////////////////////////    2018-5-7  start yzh /////////////////////
	// if(member->conference->id_senior_disp > 0)
	// {
		switch_snprintf(msg, sizeof(msg), "%s:%u", RICHTALK_SERVER_EVENT_PTT_INFOCOUNT, member->conference->count -  member->conference->senior_disp_Count);
	// }
	// else
	// {
	// 	switch_snprintf(msg, sizeof(msg), "%s:%u", RICHTALK_SERVER_EVENT_PTT_INFOCOUNT, member->conference->count);
	// }
	//	switch_snprintf(msg, sizeof(msg), "%s:%u", RICHTALK_SERVER_EVENT_PTT_INFOCOUNT, member->conference->count);
	///////////////////////////////    2018-5-7  end yzh /////////////////////


	conference_send_message(member, member->conference, msg);
}
//  ��ȡ��ǰ����Ȩ�ĳ�Ա����
void conference_loop_fn_ptt_queryfloor(conference_member_t *member, caller_control_action_t *action)
{
	char msg[128];
	conference_member_t *holder;
	switch_bool_t has_floor_member = SWITCH_FALSE;
	switch_channel_t *channel;
	switch_caller_profile_t *profile;

	if(member == NULL) return;

	holder = member->conference->ptt_floor_holder;
	if(holder && holder->session) {
		has_floor_member = SWITCH_TRUE;
		channel = switch_core_session_get_channel(holder->session);
		if ((channel == NULL) || (conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)))
		{
			return ;
		}
	...
	switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_INFOPTT, has_floor_member ? profile->caller_id_number : "null");
	conference_send_message(member, member->conference, msg);
}
...
void conference_ptt_list(conference_obj_t *conference, switch_stream_handle_t *stream)
{
	...
	///////////////////////////////    2018-5-7  start yzh /////////////////////
	// if(conference->id_senior_disp > 0)
	// {
		stream->write_function(stream, "%s:%s:%u", RICHTALK_SERVER_EVENT_PTT_INFODETAIL, conference->name, conference->count - conference->senior_disp_Count);
	// }
	// else
	// {
	// 	stream->write_function(stream, "%s:%s:%u", RICHTALK_SERVER_EVENT_PTT_INFODETAIL, conference->name, conference->count);
	// }
	///////////////////////////////    2018-5-7  end yzh /////////////////////
...
//		if (switch_test_flag(member, MFLAG_NOCHANNEL)) {  // ���Ӧ���Ǿɰ汾�ĺ��� �°汾Ӧ�������������жϵ�
		if (conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
			continue;
		}
		///////////////////////////////    2018-5-7  start yzh /////////////////////
	//	if(conference->id_senior_disp == member->id)
		if(member->senior_disp_Flag == 1)
		{
			continue;
		}
		///////////////////////////////    2018-5-7  end yzh /////////////////////
		...
		stream->write_function(stream, ":%u;%s;%s", member->id, profile->caller_id_number,
			conference->ptt_floor_holder==member ? "ptt" : "hear");
	}
...
void conference_ptt_online_list(conference_obj_t *conference, switch_stream_handle_t *stream)
{
	conference_member_t *member = NULL;
	...
	stream->write_function(stream, "%s:", RICHTALK_SERVER_EVENT_PTT_INFODETAIL);
...
//		if (switch_test_flag(member, MFLAG_NOCHANNEL)) {  // ���Ӧ���Ǿɰ汾�ĺ��� �°汾Ӧ�������������жϵ�
		...
		stream->write_function(stream, "%s,", profile->caller_id_number);
	}
...
void conference_send_all_message(conference_member_t *member, conference_obj_t *conference, const char *message)
{
	...
	for(imember = conference->members; imember; imember = imember->next) {
		if(imember->session && (channel = switch_core_session_get_channel(imember->session))) {
			if (conference_utils_member_test_flag(imember, MFLAG_NOCHANNEL)) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "  conference_send_all_message continue\n");
				continue;
			}
			profile = switch_channel_get_caller_profile(channel);
			...
			switch_event_add_body(evt, message);
			// for debug
			// switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "  conference_send_all_message content: %s  to %s\n", message,to_uri);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "  conference_send_all_message content: %s  to %s\n", message,to_uri);
...
void conference_send_all_message_memberjoin(conference_member_t *member, conference_obj_t *conference, const char *message,const char *pnumber)
{
	conference_member_t *imember;
	switch_channel_t *channel;
	switch_caller_profile_t *profile;
	...
	char to_uri_cp[256];
	char ss_content[512];
	char ss_content2[512];
	char ss_content3[512];
//2017-11-13	char ss_priorty[3];

	switch_mutex_lock(conference->mutex);
	switch_mutex_lock(conference->member_mutex);
	memset(ss_content,0x00,512);
	memset(ss_content2,0x00,512);
	memset(ss_content3,0x00,512);
//2017-11-13	memset(ss_priorty,0x00,3);
	switch_snprintf(ss_content, sizeof(ss_content), "%s:", RICHTALK_SERVER_EVENT_PTT_MEMBERS );
	switch_snprintf(ss_content2, sizeof(ss_content2), "%s:", RICHTALK_SERVER_EVENT_PTT_MEMBERS );
	switch_snprintf(ss_content3, sizeof(ss_content3), "%s:", RICHTALK_SERVER_EVENT_PTT_MEMBERS );

	for(imember = conference->members; imember; imember = imember->next) {
	//	if(imember->id == conference->id_senior_disp)
		if(imember->senior_disp_Flag == 1)
		{
			continue;
		}
		if(imember->session && (channel = switch_core_session_get_channel(imember->session))) {
...
//2017-11-13			memset(ss_priorty,0x00,3);
//2017-11-13			switch_snprintf(ss_priorty, sizeof(ss_priorty), ":%d", imember->ptt_priorty );

			if (!strcmp(pnumber, profile->caller_id_number)) 
			{
				// ˵�����
				switch_snprintf(to_uri_cp, sizeof(to_uri_cp), "%s@%s", profile->caller_id_number, switch_channel_get_variable(channel, "domain_name"));
			}
			
			
			if(strlen(ss_content) + 12 < 511)
			{
				strcat(ss_content, profile->caller_id_number);
//2017-11-13				strcat(ss_content, ss_priorty);
				strcat(ss_content, ",");
			}
			else if(strlen(ss_content2) + 12 < 511)
			{
				strcat(ss_content2, profile->caller_id_number);
//2017-11-13				strcat(ss_content, ss_priorty);
				strcat(ss_content2, ",");
			}
			else if(strlen(ss_content3) + 12 < 511)
			{
				strcat(ss_content3, profile->caller_id_number);
//2017-11-13				strcat(ss_content, ss_priorty);
				strcat(ss_content3, ",");
			}
		}
	...
	if(strlen(ss_content) > 32)
	{
		...
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from", RICHTALK_SERVER_EVENT_SMS_FROM);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "from_full", RICHTALK_SERVER_EVENT_SMS_FROM);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "to", to_uri_cp);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "type", "text/plain");
		switch_event_add_body(evt, ss_content);
		// for debug
		// switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content,to_uri_cp);

		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content,to_uri_cp);
		switch_event_fire(&evt);
	}
	else if(strlen(ss_content2) > 32)
	{
		switch_event_create_subclass(&evt, SWITCH_EVENT_CUSTOM, "SMS::SEND_MESSAGE");
		//switch_event_create(&evt, SWITCH_EVENT_MESSAGE);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "proto", "GLOBAL_SMS");
		...
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "to", to_uri_cp);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "type", "text/plain");
		switch_event_add_body(evt, ss_content2);
		// for debug
		// switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content2,to_uri_cp);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content2,to_uri_cp);

		switch_event_fire(&evt);
	}
	else if(strlen(ss_content3) > 32)
	{
		switch_event_create_subclass(&evt, SWITCH_EVENT_CUSTOM, "SMS::SEND_MESSAGE");
		//switch_event_create(&evt, SWITCH_EVENT_MESSAGE);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "proto", "GLOBAL_SMS");
		...
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "to", to_uri_cp);
		switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "type", "text/plain");
		switch_event_add_body(evt, ss_content3);
		// for debug
		// switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content3,to_uri_cp);
		switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_INFO, "conference_send_all_message content: %s to %s\n", ss_content3,to_uri_cp);
		switch_event_fire(&evt);
	}
...
// 给高级调度员发当前在线的所有成员
void conference_send_all_message_memberjoin_for_senior(conference_member_t *member, conference_obj_t *conference, const char *message,const char *pnumber)
{
	conference_member_t *imember;
	switch_channel_t *channel;
	switch_caller_profile_t *profile;
...
void conference_send_message(conference_member_t *member, conference_obj_t *conference, const char *message)
{
	switch_channel_t *channel;
	switch_caller_profile_t *profile;
	switch_event_t *evt = NULL;
	...
	if(member->session && (channel = switch_core_session_get_channel(member->session))) {
		if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
			profile = switch_channel_get_caller_profile(channel);
			switch_snprintf(to_uri, sizeof(to_uri), "%s@%s", profile->caller_id_number, switch_channel_get_variable(channel, "domain_name"));
			switch_event_create_subclass(&evt, SWITCH_EVENT_CUSTOM, "SMS::SEND_MESSAGE");
			...
			switch_event_add_header_string(evt, SWITCH_STACK_BOTTOM, "type", "text/plain");
			switch_event_add_body(evt, message);
	//		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member->session), SWITCH_LOG_INFO, "conference_send_message: %s to %s\n", message, to_uri);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "conference_send_message: %s to %s\n", message, to_uri);
			switch_event_fire(&evt);
		}
	}
}
...
{
	if(member == NULL || !conference_utils_member_test_flag(member, MFLAG_CAN_SPEAK))
		return;
	conference_api_sub_mute(member, NULL, NULL);
	//////////////////  2017-4-18 start  //////////////////////////

//	if(member->conference->id_senior_disp == member->id)
	if(member->senior_disp_Flag == 1)
	{
		return;
	}
	if(member->conference->ptt_floor_holder == member)   //yield floor control  
	{  // �ͷŻ���Ȩ
			conference_api_sub_ptt_floor(member, NULL, NULL);
	}
	//////////////////  end   /////////////////////////////////////
...
void conference_loop_mute_off(conference_member_t *member, caller_control_action_t *action)
{
	...
	if(member == 0x0)
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,  "2017-11-16  conference_loop_mute_off  member == 0x00\n");
		return;
	}
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,  "2017-11-16  conference_loop_mute_off  member != 0x00\n");
	///////////////////    2017-11-16    end  ///////////////////////
...
//	if(member->conference->id_senior_disp == member->id)
	...
	if(member->conference->ptt_floor_holder != member)   //try to get floor control
		conference_api_sub_ptt_floor(member, NULL, NULL);
	if(member->conference->ptt_floor_holder == member) {
		conference_api_sub_unmute(member, NULL, NULL);
		if (!conference_utils_member_test_flag(member, MFLAG_CAN_HEAR)) {
			conference_api_sub_undeaf(member, NULL, NULL);
		}
...
void conference_loop_event(conference_member_t *member, caller_control_action_t *action)
{
	...
	if (test_eflag(member->conference, EFLAG_DTMF) && switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		...
		if(member == 0x0)
		{
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,  "2017-11-16  conference_loop_event  member == 0x00\n");
			return;
		}
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,  "2017-11-16  conference_loop_event  member != 0x00\n");
		///////////////////    2017-11-16    end  ///////////////////////
...
void conference_loop_output(conference_member_t *member)
{
	...
	if ((call_list = switch_channel_get_private(channel, "_conference_autocall_list_"))) {
		...
		//////////////  17-4-15  start ///////////////
	//	const char *flags = switch_channel_get_variable(channel, "conference_utils_auto_outcall_flags");  // ����� 1.9 �汾�� ���ڰ���������ĳ� 1.7 ��
		const char *flags = switch_channel_get_variable(channel, "conference_auto_outcall_flags");
		const char *sgroup_priorty = switch_channel_get_variable(channel, "conference_auto_outcall_group_priorty"); // ��ȡ������ȼ�
		const char *spoc_number = switch_channel_get_variable(channel, "richtalk_call_number"); // ���淢�������ն˺���
	...
	//	const char *skip_member_beep = switch_channel_get_variable(channel, "conference_auto_outcall_skip_member_beep");
		...
		////////////////////////////////  2017-5-26  start   ///////////////////////////////
		// switch_str_nil( sgroup_priorty );
		if (!zstr(sgroup_priorty)) 
	//	if(strcmp(sgroup_priorty , ""))
		{
			member->group_priorty = atoi(sgroup_priorty);  // ������ȼ�
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
			"sgroup_priorty.   value =  %d\n", member->group_priorty);
			
			if(member->conference->group_priorty != member->group_priorty)
			{
				member->conference->group_priorty = member->group_priorty;
			}
		}
		else
		{
			member->group_priorty = 1;
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
			"sgroup_priorty.  no value\n");
		}
		////////////////////////////////////   2017-5-26   end   ///////////////////////////
		...
		////////////////////////////////////   2017-11-21   start �����һ���˷������ĺ���   ///////////////////////////
		if ((!zstr(spoc_number)) && (strlen(member->conference->poc_call_number) == 0)) 
		{
			switch_snprintf(member->conference->poc_call_number, sizeof(member->conference->poc_call_number), "%s", spoc_number );
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"2017-11-21  Member of the first group call  member->conference->poc_call_number = %s  spoc_number = %s \n", member->conference->poc_call_number, spoc_number);
		}   
		////////////////////////////////////   2017-11-21   end   ///////////////////////////
	...
	{
			const char *spoc_number = switch_channel_get_variable(channel, "richtalk_call_number"); // ���淢�������ն˺���
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-11-28  �����ʱ�������");
			if ((!zstr(spoc_number)) && (strlen(member->conference->poc_call_number) == 0)) 
			{
				switch_snprintf(member->conference->poc_call_number, sizeof(member->conference->poc_call_number), "%s", spoc_number );
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
