...
void conference_member_update_status_field(conference_member_t *member)
{
	...
	cJSON *json, *audio, *video;
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "18-4-24   18-4-2418-4-2418-4-24 33333333333\n");
...
switch_status_t conference_member_add(conference_obj_t *conference, conference_member_t *member)
{
	...
	////////////////////////////           2017-4-20  start          //////////////////////////////
	//向所有成员发送新成员加入事件.
     // 2017-8-28  还差要向新进来的成员发送之前已经在会议的成员
	if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
		channel = switch_core_session_get_channel(member->session);
		prof = switch_channel_get_caller_profile(channel);
		
		switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_MEMBERJOIN, prof->caller_id_number);
		conference_send_message(member, conference, msg);

	//	if(strcmp(prof->caller_id_number, conference->senior_disp))
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "18-5-14   member->senior_disp_Flag  =  %d \n", member->senior_disp_Flag);
		if(member->senior_disp_Flag == 0 )
		{  // 说明 加入的这个成员不是高级调度员
		//	switch_snprintf(msg, sizeof(msg), "%s:%s:%d", RICHTALK_SERVER_EVENT_PTT_MEMBERJOIN, prof->caller_id_number, member->ptt_priorty);
			
			conference_send_all_message(member, conference, msg);
			
	//		if((conference->count > 2) || (conference->count > 1 &&  conference->id_senior_disp == 0 )) 
			if((conference->count - conference->senior_disp_Count > 1 )) 
			{
				conference->m_b_count = 1;   // 用来标记这次会议有没有超过 大于等于2个人      == 1 有  == 0  没有
				conference_send_all_message_memberjoin(member, conference, msg,prof->caller_id_number);
				// 在这里把如果这个会议只剩下一个成员，则马上退出该会议 把last_member_exiting_time 的值设置为2秒
				conference->last_member_exiting_time = 2;
			}
		}
		else
		{   // 说明 加入的这个成员是高级调度员    
			// 好像下面的代码没有必要执行
	//		conference->id_senior_disp = member->id;
			if(conference->count > 2)
			{
				conference->m_b_count = 1;   // 用来标记这次会议有没有超过 大于等于2个人      == 1 有  == 0  没有
				conference_send_all_message_memberjoin_for_senior(member, conference, msg,prof->caller_id_number);
				// 在这里把如果这个会议只剩下一个成员，则马上退出该会议 把last_member_exiting_time 的值设置为2秒
	//			conference->last_member_exiting_time = 2;
			}
	...
	/////////////////////////      2017-5-27	start   判断是否能够说话，能说话就把禁言 ///////////////////
	if (conference_utils_member_test_flag(member, MFLAG_CAN_SPEAK)) 
	{
...
void conference_member_set_floor_holder(conference_obj_t *conference, conference_member_t *member)
{
	...
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "18-4-24   18-4-2418-4-2418-4-24 1111111111111111   member == NULL\n");
	}
	...
	if (conference->floor_holder) {
		if (conference->floor_holder == member) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "17-10-24   has positional audio blocked.666666666666666\n");
			if(old_member == NULL)
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "17-10-24   has positional audio blocked.666666666666666 old_member == NULL \n");
			}
			if(old_member->channel == NULL)
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "17-10-24   has positional audio blocked.77777777777777777 old_member == NULL \n");
				return ;  // 2017-10-24 这里一定要返回 
			}
	//		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Dropping floor %s\n",
	...
	if (member) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
									  "17-10-24   has positional audio blocked.777777777777\n");
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
									  "18-4-24   18-4-2418-4-2418-4-24 2222222222222\n");
...
switch_status_t conference_member_del(conference_obj_t *conference, conference_member_t *member)
{
	...
	char sc_number[100] = {0};  // 存放退出成员终端号码
	switch_channel_t *channel;
	switch_caller_profile_t *caller_profile;
	switch_bool_t flag_lost_ptt_floor = SWITCH_FALSE;
	//////////////////////       end         ///////////////////
	...
	if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
		if(member->session) {
			if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
				...
				switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_ENDTALK, caller_profile->caller_id_number);
				switch_snprintf(msg2, sizeof(msg2), "%s:%s", RICHTALK_SERVER_EVENT_PTT_MEMBERQUIT, caller_profile->caller_id_number);
				switch_snprintf(sc_number, sizeof(sc_number), "%s", caller_profile->caller_id_number);
			}
		}
	}
	else
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-11-10 conference_member_del not MFLAG_NOCHANNEL\n");
	}
	/////////////////////////        end     //////////////////////
...
//	if (member == member->conference->floor_holder) {    //////   \D5\E2\C0\EF\D3\D0\D0޸\C4
	if ((member == member->conference->ptt_floor_holder) && (member->conference->count > 1) &&  (member->conference->floor_holder == member)) {    //////   \D5\E2\C0\EF\D3\D0\D0޸\C4
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "17-10-24 member == member->conference->ptt_floor_holder conference_member_set_floor_holder\n");
		conference_api_sub_unmute(member, NULL, NULL);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "18-4-25 member number = %s member->conference->count = %d\n",sc_number, member->conference->count);
		conference_member_set_floor_holder(member->conference, NULL);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "17-10-24 flag_lost_ptt_floor && !zstr(msg)\n");
		conference_send_all_message(member, conference, msg);
	//////////////////////   2017-4-20   start ////////////////////
	//	flag_lost_ptt_floor = SWITCH_TRUE;
	//////////////////////   end       ////////////////////
	}
	...
	//////////////////////  2017-4-20   start    /////////////////////
	if(flag_lost_ptt_floor && !zstr(msg)) 
	{
//		conference_send_all_message(NULL, conference, msg);
//	17-10-24	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "17-10-24 flag_lost_ptt_floor && !zstr(msg)\n");
//	17-10-24	conference_send_all_message(member, conference, msg);
	}
//	if (!conference_utils_member_test_flag(member, MFLAG_NOCHANNEL)) {
//	if(!zstr(msg2) && member->id != conference->id_senior_disp) 
	if(!zstr(msg2) && member->senior_disp_Flag == 0)  //  说明不是高级调度员
	{
		
	//	switch_snprintf(msg, sizeof(msg), "%s:%s", RICHTALK_SERVER_EVENT_PTT_MEMBERQUIT, caller_profile->caller_id_number);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "msg2\n");
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "msg2 = %s\n",msg2);
//		conference_send_all_message(NULL, conference, msg2);
		conference_send_all_message(member, conference, msg2);
		
	}
	
	////////////////////////////       end      ///////////////////////   */

	// 程序运行到这里之后，估计 组里面的成员数应该已经把这个人减去了

	// if(conference->id_senior_disp == member->id)
	// {
	// 	conference->id_senior_disp = 0;
	// }

//	if(conference->count > 1) 
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "18-5-15  conference->count = %d   conference->senior_disp_Count = %d\n",  conference->count , conference->senior_disp_Count);

	if(conference->count - conference->senior_disp_Count > 1) 
	{
		if(!strcmp(sc_number, conference->poc_call_number))
		{  // 说明是发起组呼的终端号码
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "sc_number = %s conference->poc_call_number =%s\n",sc_number , conference->poc_call_number);
			conference_utils_set_flag(conference, CFLAG_DESTRUCT); 
		}
/*		else
		{
			if(conference->count -  <= 2 && conference->id_senior_disp == member->id)
			{
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2018-4-28 conference->count <= 2 msg2 = %s\n",msg2);
				conference_utils_set_flag(conference, CFLAG_DESTRUCT); 
			}
		}
*/		
	}
	else  // 2017-11-28 说明当前会议已经没有成员了，则发送自己退出的消息给自己  发送这个消息主要是因为android那的显示问题
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "2017-11-28 conference->count < 1 msg2 = %s\n",msg2);
		conference_send_message(member, conference, msg2);
	}
