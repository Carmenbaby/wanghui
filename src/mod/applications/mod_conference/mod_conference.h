...
const char RICHTALK_SERVER_EVENT_SMS_FROM[] = "SERVER_MESSAGE@RICHTALK";
const char RICHTALK_SERVER_EVENT_PTT_BEGINTALK[] = "RICHTALK_PTT_BEGINTALK";
const char RICHTALK_SERVER_EVENT_PTT_ENDTALK[] = "RICKTALK_PTT_ENDTALK";
const char RICHTALK_SERVER_EVENT_PTT_MEMBERJOIN[] = "RICHTALK_PTT_MEMBERJOIN";
const char RICHTALK_SERVER_EVENT_PTT_MEMBERQUIT[] = "RICHTALK_PTT_MEMBERQUIT";
const char RICHTALK_SERVER_EVENT_PTT_INFODETAIL[] = "RICHTALK_PTT_INFO_DETAIL";
const char RICHTALK_SERVER_EVENT_PTT_INFOCOUNT[] = "RICHTALK_PTT_INFO_COUNT";
const char RICHTALK_SERVER_EVENT_PTT_INFOPTT[] = "RICHTALK_PTT_INFO_PTT";
////////////2017-4-13  end  //////////*/
...
typedef struct conference_obj {
	...
	///////////////////// 2017-4-13 start  /////////////////
	//当前说话成员
	conference_member_t *ptt_floor_holder;
	// 只剩最后一个成员的时间
	switch_time_t ptt_last_member_time;
	// 配置参数
	uint32_t lost_ptt_on_silence;
	uint32_t lost_ptt_on_talking;
	uint32_t last_member_exiting_time;
	// http命令接口密钥
	char *http_interface_key;
	uint32_t dummy_frame_send_rate;
	switch_mutex_t *check_member_mutex;
	switch_status_t check_status;
	char poc_call_number[32];  // richtalk_call_number 保存第一个人发起组呼的号码
	char senior_disp[32];  // 保存高级调度员号码 Senior dispatcher.  (senior_dispatcher)
	uint32_t senior_disp_Count;  // 保存高级调度员总数 Senior dispatcher.  (senior_dispatcher)
	uint32_t id_senior_disp;  // 保存高级调度员号码 对应在conference里面的id值  Senior dispatcher.  (senior_dispatcher)
	switch_bool_t m_b_count; // 用来标记这次会议有没有超过 大于等于2个人      == 1 有  == 0  没有

	switch_bool_t m_b_singecall; // 用来标记这次会议 是不是单呼  1 - 单呼  0 - 不是单呼
	////////////////////       end       ///////////////////
	////////////////////////////////////////////////////////////////

	////////////////////////////////////////////////////////////////
	////////////////////////  2017-5-26 start //////////////
	switch_time_t checkMember_time;  // 查询当前所有会议成员以及以及它们优先级
	uint32_t group_priorty;  // 组间优先级
	switch_channel_t *channel;  // 用于组内优先级的
	////////////////////////////    end    /////////////////
...
struct conference_member {
	...
	int ptt_priorty;
	// 得到话语权的时间
	switch_time_t ptt_gain_time;
//	switch_time_t last_talking;  // 这个已经存在了
	
	uint32_t group_priorty;  // 组间优先级

	// 高级调度员标志位   1 - 是高级调度员   0 - 不是
	int senior_disp_Flag;
	////////////////////       end       ///////////////////
	/////////////////////////////////////////////////////
...
// 判断当前号码的优先级，是否比较高，如果比较高 返回 SWITCH_STATUS_SUCCESS 0 否则返回 1 
